...
const { Normal } = require('../lib/stats-normal');
...
describe('Normal', () => {
	...
	describe('non-standard', () => {
		...
		describe('cdf', () => {
			let tests = [[0.5, 0.1586], [1, 0.5], [1.5, 0.8413], [2, 0.9772]];
			for (let [input, expected] of tests) {
				it(`should have correct cdf for x = ${input}`, () => {
		...
		describe('pdf', () => {
			let tests = [[0.5, 0.4839], [1, 0.7978], [1.5, 0.4839], [2, 0.1079]];
			for (let [input, expected] of tests) {
				it(`should have correct cdf for x = ${input}`, () => {
